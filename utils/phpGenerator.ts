...
export const getPluginFileContent = (config: AppConfig): string => {
  return `<?php
/**
...
class PolyGlotTranslator {
    ...
    private function __construct() {
        ...
        add_action('wp', [$this, 'determine_language']);
        add_action('wp_enqueue_scripts', [$this, 'enqueue_assets']);
        
        // Translation Hooks - Priority 99999 ensures we run AFTER page builders (Elementor, Divi, etc.)
        add_filter('the_content', [$this, 'translate_content_hook'], 99999);
        add_filter('the_title', [$this, 'translate_title_hook'], 10, 2);
        add_filter('widget_text', [$this, 'translate_content_hook'], 99999);
    ...
    public function sanitize_settings($input) {
        ...
        $output = [];
        $output['provider'] = sanitize_text_field($input['provider']);
        $output['google_key'] = sanitize_text_field($input['google_key']);
        $output['deepl_key'] = sanitize_text_field($input['deepl_key']);
        ...
        flush_rewrite_rules();
        // Clear transients/cache on save
        global $wpdb;
        $wpdb->query("DELETE FROM $wpdb->options WHERE option_name LIKE '_transient_pg_tr_%'");
    ...
    public function render_admin_page() {
        ...
        $google_key = isset($opts['google_key']) ? $opts['google_key'] : '';
        $deepl_key = isset($opts['deepl_key']) ? $opts['deepl_key'] : '';
                                ...
                                <div class="pg-card-body">
                                    <div class="pg-form-group">
                                        ...
                                        <select name="polyglot_settings[default_lang]" class="pg-input" style="max-width:300px;">
                                            <?php foreach($languages as $code => $details): ?>
                                                <option value="<?php echo $code; ?>" <?php selected($default_lang, $code); ?>>
                                                    <?php echo $details[1] . ' ' . $details[0]; ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <p style="font-size:12px; color:#64748b; margin-top:4px;">Detected from WordPress: <strong><?php echo $detected_lang; ?></strong>. Save changes if you modify this.</p>
                                    </div>
                                    ...
                                    <div class="pg-form-group">
                                        ...
                                        <div class="pg-lang-grid">
                                            ...
                                            ?>
                                            <div class="pg-lang-item <?php echo $item_class; ?>" id="card-<?php echo $code; ?>">
                                                <div style="display:flex; align-items:center; gap:10px;">
                                                    <span style="font-size:20px;"><?php echo $details[1]; ?></span>
                                                    <div>
                                                        <div style="font-size:14px; font-weight:600; color:#1e293b;"><?php echo $details[0]; ?></div>
                                                        <div style="font-size:11px; color:#94a3b8;"><?php echo strtoupper($code); ?></div>
                                                    </div>
                                                </div>
                                                <label class="pg-toggle">
                                                    <input type="checkbox" 
                                                           ...
                                                           onchange="toggleLangCard(this, '<?php echo $code; ?>')">
                             ...
                             <div class="pg-card">
                                ...
                                <div class="pg-card-body">
                                    ...
                                    <div id="config-google" class="pg-form-group" style="display: <?php echo $provider === 'google' ? 'block' : 'none'; ?>">
                                        ...
                                        <input type="password" name="polyglot_settings[google_key]" value="<?php echo esc_attr($google_key); ?>" class="pg-input" placeholder="AIza...">
                                        <p style="font-size:12px; color:#64748b; margin-top:4px;">Supports standard Google Cloud Translation API (v2).</p>
                                    </div>
                                    ...
                                    <div id="config-deepl" class="pg-form-group" style="display: <?php echo $provider === 'deepl' ? 'block' : 'none'; ?>">
                                        ...
                                        <input type="password" name="polyglot_settings[deepl_key]" value="<?php echo esc_attr($deepl_key); ?>" class="pg-input" placeholder="...:fx">
                                        <p style="font-size:12px; color:#64748b; margin-top:4px;">Supports Free (:fx) and Pro keys.</p>
                                    </div>
                                </div>
                             </div>
                        </div>
    ...
    public function determine_language() {
        if (is_admin()) return; 

        $lang = get_query_var('pg_lang');
    ...
    public function handle_seo_redirects() {
        ...
        if (isset($_GET['lang']) && !get_query_var('pg_lang')) {
            ...
            if (in_array($lang, $this->active_languages)) {
                $new_url = home_url('/' . $lang . $_SERVER['REQUEST_URI']);
    ...
    private function get_translation($text, $lang) {
        ...
        $cache_key = 'pg_tr_' . substr(md5($text . $lang . $provider), 0, 24);
        
        // BYPASS CACHE FOR ADMINS TO FIX SETUP ISSUES
        ...
        if ($translated && $translated !== $text && strpos($translated, 'PG DEBUG') === false) {
    ...
    private function translate_with_google($text, $lang) {
        $key = isset($this->options['google_key']) ? $this->options['google_key'] : '';
        if (!$key) return $text . ' <!-- PG DEBUG: No API Key Configured -->';
        ...
        // IMPORTANT: 'sslverify' => false fixes issues on hosts with old certificates (curl error 60)
        ...
        if (is_wp_error($response)) {
            error_log('PolyGlot API Error: ' . $response->get_error_message());
            return $text . ' <!-- PG DEBUG: ' . esc_html($response->get_error_message()) . ' -->';
        }
        ...
        if (isset($data['error'])) {
             // Return original text + debug comment
             return $text . ' <!-- PG DEBUG: ' . esc_html($data['error']['message']) . ' -->';
        }
    ...
    private function translate_with_deepl($text, $lang) {
        $key = isset($this->options['deepl_key']) ? $this->options['deepl_key'] : '';
        if (!$key) return $text;
        ...
        if (is_wp_error($response)) return $text . ' <!-- PG DEBUG: DeepL Connect Error -->';
        ...
        return $text . ' <!-- PG DEBUG: DeepL returned no text -->';
    }
    ...
    public function enqueue_assets() {
        ...
        wp_add_inline_style('polyglot-style', '
            ...
            /* Inline Style - PURE INLINE to avoid layout issues */
            .pg-inline { display: inline-block; white-space: nowrap; margin: 0; padding: 0; line-height: 1; }
            .pg-inline .pg-switcher-dropdown { display: none; }
            .pg-inline-list { display: inline; margin: 0; padding: 0; list-style: none !important; }
            .pg-inline-item { display: inline-block !important; margin: 0 4px !important; padding: 0 !important; list-style: none !important; }
            .pg-inline-item a { 
    ...
    public function render_language_switcher($atts) {
        ...
        if ($a['style'] === 'inline') {
            ...
            foreach ($all_langs as $code) {
                ...
                $output .= '<a href="' . $url . '" class="' . $active_class . '">' . $flag . ' ' . strtoupper($code) . '</a>';
                $output .= '</span>';
            }
            $output .= '</span></span>';
            return $output;
        ...
        foreach ($all_langs as $code) {
            ...
            $output .= '<a href="' . $url . '" class="pg-item ' . $active_class . '">';
            $output .= '<span class="pg-flag">' . $flag . '</span> ' . strtoupper($code);
            $output .= '</a>';
        }
        $output .= '</div>';
        $output .= '<div class="pg-switcher-btn">';
        $output .= '<span class="pg-flag">' . $current_flag . '</span> ' . strtoupper($this->current_lang);
